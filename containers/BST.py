...
class BST(BinaryTree):
    ...
    def _is_bst_satisfied(node):
        ...
        less than/greater than, rather than ensuring that all nodes
        to the left/right are less than/greater than.
    ...
    def _remove(value, node):
        ...
        if node.value == value:
            if node.left is None:
                ...
                min_of_right = BST._find_smallest(node.right)
                node.value = min_of_right
                node.right = BST._remove(min_of_right, node.right)
                return node
